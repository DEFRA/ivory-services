...
lab.experiment(TestHelper.getFile(__filename), () => {
  ...
  lab.test('Payment data validates correctly', async () => {
    const { value } = Payment.validate(data, { abortEarly: false })
  ...
  Object.keys(payment).forEach((field) => {
    lab.test(`Payment data invalidates ${field} correctly`, async () => {
      data[field] = false
      const { error } = Payment.validate(data, { abortEarly: false })
      switch (field) {
        case 'amount':
  ...
  lab.test('Payment parameter validate correctly', async () => {
    data.id = 'abc'
    const { error } = Payment.validateParams(data, { abortEarly: false })
